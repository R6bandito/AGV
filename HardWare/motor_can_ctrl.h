...
/* —————————————————————————————————————————————————————— */
typedef      bool       RMOTOR_B;
typedef      uint8_t    RMOTOR_U8;
typedef      int8_t     RMOTOR_I8;
typedef      uint16_t   RMOTOR_U16;
typedef      uint32_t   RMOTOR_U32;
typedef      int32_t    RMOTOR_I32;
typedef      float      RMOTOR_FLOT;
/* —————————————————————————————————————————————————————— */
...
typedef enum                        // 错误码枚举.
{
    ...
    MOTOR_ERR_MUTEX_TIMEOUT     = 9,    // 锁超时(SYS启用情况下).
...
typedef struct 
{
    RMOTOR_U8     nodeId;                /* 节点ID. */
    RMOTOR_U16    statusWord;           /* 状态字. */
    MotorMode_t   runingMode;        /* 当前运行模式. */
    RMOTOR_U16    lastErrorCode;        /* 上一次返回的错误码. */

    RMOTOR_U8     is_Enable;             /* 判断电机是否上电. */
    RMOTOR_U8     is_Fault;              /* 判断是否发生故障. */
    RMOTOR_U8     is_TargetReach;        /* 位置是否到达标志. */
    RMOTOR_U8     is_InternalLimit;      /* 限位触发标志位. */

    /* 用于转向驱动电机. */
    RMOTOR_U16    actual_AngleUnits;     /* 0~65535. (0~360°) */
    RMOTOR_FLOT   actual_AngleDeg;          /* 当前实际角度. */

    /* 用于行走电机. */
    RMOTOR_I32    total_Revolutions;      /* 轮子运转总圈数. */
    RMOTOR_FLOT   actual_DistanceMM;        /* 当前实际行走距离. 单位mm */   
    RMOTOR_I32    current_Rpm;            /* 直行电机当前运行速度. */
...
/* —————————————————————————————————————————————————————— */
                    /* FEATURE */
    #define MOTOR_USE_SYS                         (0)
        #if (MOTOR_USE_SYS)
            __WEAK void MotorOS_Init( RMOTOR_U8 needMutexNum );                             /* 带OS的初始化. 内部需要初始化 needMutexNum 把独立锁. */

            __WEAK RMOTOR_B MotorOS_Lock( RMOTOR_U8 slots, RMOTOR_U32 timeouts );           /* 对设备表中指定索引的电机加锁. 成功加锁返回True(1). */

            __WEAK RMOTOR_B MotorOS_Unlock( RMOTOR_U8 slots );                              /* 对设备表中指定索引的电机解锁. 成功解锁返回True(1). */

            __WEAK void MotorOS_Delay( RMOTOR_U32 delay_ms );                               /* 操作系统延时映射. */

            /* 锁获取超时时间. */
            #define MOTOR_OS_MUTEX_TIMEOUT_MS     (200)

        #endif /* MOTOR_USE_SYS */

        #if (MOTOR_USE_SYS)
        #define MOTOR_API_ENTER_RET(id, type)                                    \
            do {                                                                 \
                if (!motorOS_Lock((id), (type)))  return -(RMOTOR_I8)MOTOR_ERR_MUTEX_TIMEOUT; \
            } while(0)

        #define MOTOR_API_ENTER_VOID(id, type)                                  \
            do {                                                                \
                if (!motorOS_Lock((id), (type))) {                            \
                Cus_Motor_ErrorHook((id), (type), MOTOR_ERR_MUTEX_TIMEOUT);   \
                return;                                                       \
              }                                                               \
            } while(0)
        
        #define MOTOR_API_EXIT(id, type)                                     \
            do {                                                             \
                if (!motorOS_UnLock((id), (type)))                           \
                    Cus_Motor_ErrorHook((id), (type), MOTOR_ERR_MUTEX_TIMEOUT);\
            } while(0)
        
        #else
        #define MOTOR_API_ENTER_RET(id, type)    (void)0
        #define MOTOR_API_ENTER_VOID(id, type)   (void)0
        #define MOTOR_API_EXIT(id, type)         (void)0
        #endif
/* —————————————————————————————————————————————————————— */
...
/* —————————————————————————————————————————————————————— */
    ...
    #if (MOTOR_USE_SYS)
        /* 每个电机一把锁. */
        #define MOTOR_OS_MUTEX_NUM                (MOTOR_MAX_SUPPORT_STEER + MOTOR_MAX_SUPPORT_TRAC)
    #endif /* MOTOR_USE_SYS */
